...
export function usePosition(panelEl: Ref<HTMLElement | undefined>) {
  ...
  const isHidden = computed(() => {
    ...
    return (
      !isDragging.value && !state.value.open && !isHovering.value && !isTouchDevice && state.value.minimizePanelInactive
    );
  });
  ...
  const iframeStyle = computed(() => {
    ...
    switch (state.value.position) {
      ...
      case 'bottom':
        ...
        if (anchorX - frameMargin.left < width / 2) style.left = `${width / 2 - anchorX + frameMargin.left}px`;
      ...
      case 'left':
        ...
        if (anchorY - frameMargin.top < height / 2) style.top = `${height / 2 - anchorY + frameMargin.top}px`;
  ...
  const panelStyle = computed(() => {
    const style: any = {
      transform: isVertical.value
        ? `translate(${
            isHidden.value ? `calc(-50% ${state.value.position === 'right' ? '+' : '-'} 15px)` : '-50%'
          }, -50%) rotate(90deg)`
        : `translate(-50%, ${
            isHidden.value ? `calc(-50% ${state.value.position === 'top' ? '-' : '+'} 15px)` : '-50%'
          })`,
    };
    if (isHidden.value) {
      switch (state.value.position) {
