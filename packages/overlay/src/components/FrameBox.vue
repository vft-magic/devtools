...
import { onRpcSeverReady, rpcServer } from '@tmagic/devtools-core';
...
watchEffect(() => {
  ...
  if (state.value.open) {
    ...
    if (Array.from(container.value.children).every((el) => el !== iframe)) container.value.appendChild(iframe);
  }
});

useEventListener(window, 'keydown', (_e) => {
...
useEventListener(window, 'mousedown', (e: MouseEvent) => {
  ...
  const matched = e.composedPath().find((_el) => {
    ...
    return (
      Array.from(el.classList || []).some((c) => c.startsWith('tmagic-devtools')) ||
...
const viewModeClass = computed(() => {
  if (props.viewMode === 'xs') return 'view-mode-xs';
  if (props.viewMode === 'fullscreen') return 'view-mode-fullscreen';
  return '';
});
</script>

<template>
  <div v-show="state.open" ref="container" class="tmagic-devtools-frame" :class="viewModeClass">
    <!-- Handlers -->
    <div
      v-show="state.position !== 'top'"
      class="tmagic-devtools-resize tmagic-devtools-resize--horizontal"
      :style="{ top: 0 }"
    ...
    <div
      ...
      class="tmagic-devtools-resize tmagic-devtools-resize--vertical"
      :style="{ left: 0 }"
    ...
    <div
      ...
      class="tmagic-devtools-resize tmagic-devtools-resize-corner"
...
.tmagic-devtools-frame {
  ...
  :deep(iframe) {
    ...
    height: 100%;
    outline: none;
    background: var(--tmagic-devtools-widget-bg);
