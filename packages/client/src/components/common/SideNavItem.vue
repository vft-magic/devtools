...
const tabPath = computed(() =>
  'path' in props.tab
    ? `${props.tab.path.startsWith('/') ? `` : `/`}${props.tab.path!}`
    : `/custom-tab-view/${(props.tab as ModuleBuiltinTab).name}`,
);
...
function onClick() {
  ...
  if ('onClick' in props.tab && props.tab.onClick) props.tab.onClick();
  else if (props.target === 'side') devtoolsClientState.value.splitScreen.view = props.tab.name;
}
</script>
...
<template>
  <VueTooltip :disabled="!minimized" placement="right" :class="{ 'w-full': !minimized }">
    <component
      ...
      op65
      :disabled="disabled"
      :class="[disabled ? 'cursor-not-allowed op40!' : 'hover:(bg-active op-100)']"
      :w="minimized ? '10' : 'full'"
    ...
    >
      <div flex="~ items-center gap-3">
        <TabIcon text-xl :icon="tab.icon" :fallback="tab.fallbackIcon" :title="tab.name" :show-title="false" />
