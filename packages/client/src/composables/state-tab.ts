...
import { useDevToolsState } from '@tmagic/devtools-core';
import type { CustomTab } from '@tmagic/devtools-kit';

import { devtoolsClientState } from './state';

import { getBuiltinTab } from '~/constants/tab';
...
export function useAllTabs() {
  ...
  const allTabs = computed(() => {
    ...
    const tabs = [...getBuiltinTab()];
    customTabs.value.forEach((tab) => {
      const currentTab: [string, Array<ModuleBuiltinTab | CustomTab>] | undefined = tabs.find(
        (t) => t[0] === tab.category,
      );
      if (currentTab) {
        if (currentTab[1].some((t) => t.name === tab.name)) return;

        if (!vitePluginDetected || isInElectron || isInChromePanel) return;
  ...
  const flattenedTabs = computed(() =>
    allTabs.value.reduce((prev, [, tabs]) => {
      tabs.forEach((tab) => {
        prev.push(tab);
      });
      return prev;
  ...
  const categorizedTabs = computed(() => {
    ...
    // TODO: custom tabs
    const tabs = allTabs.value.reduce<CategorizedTabs>(
      (prev, [category, tabs]) => {
  ...
  const enabledTabs = computed(() =>
    categorizedTabs.value.reduce<CategorizedTabs>((prev, [meta, tabs]) => {
      if (meta.hidden) return prev;
      const filtered = tabs.filter((t) => !t.hidden);
      if (filtered.length) prev.push([meta, filtered]);
      return prev;
    }, []),
  );
  const enabledFlattenTabs = computed(() =>
    enabledTabs.value.reduce((prev, [, tabs]) => {
      tabs.forEach((tab) => {
        prev.push(tab);
      });
      return prev;
...
export function getCategorizedTabs(
  flattenTabs: MaybeRef<(CustomTab | ModuleBuiltinTab)[]>,
  enabledTabs: MaybeRef<CategorizedTabs>,
) {
  return computed<CategorizedTabs>(() => {
    const categories: CategorizedTabs = [];
    const { pinnedTabs } = devtoolsClientState.value.tabSettings;
    const tabs = toValue(enabledTabs).reduce<{ tab: CategorizedTab; category: CategorizedCategory }[]>(
      (prev, [{ name: cateName, hidden }, tabs]) => {
        tabs.forEach((tab) => {
    ...
    tabs.forEach(({ tab, category }) => {
      ...
      if (!cates) categories.push([category, [tab]]);
      else cates[1].push(tab);
    });
    const pinned = categories.find(([{ name }]) => name === 'pinned');
    if (pinned) pinned[1].sort((a, b) => pinnedTabs.indexOf(a.name) - pinnedTabs.indexOf(b.name));
    return categories;
  });
}
