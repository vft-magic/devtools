...
target.__TMAGIC_DEVTOOLS_COMPONENT_INSPECTOR_ENABLED__ ??= true;
export function toggleComponentInspectorEnabled(enabled: boolean) {
  target.__TMAGIC_DEVTOOLS_COMPONENT_INSPECTOR_ENABLED__ = enabled;
}

function waitForInspectorInit(cb: () => void) {
  let total = 0;
  const timer = setInterval(() => {
    if (target.__TMAGIC_INSPECTOR__) {
      clearInterval(timer);
      total += 30;
      cb();
    }
    if (total >= /* 5s */ 5000) clearInterval(timer);
  }, 30);
}

function setupInspector() {
  const inspector = target.__TMAGIC_INSPECTOR__;
...
export function getComponentInspector(): Promise<ComponentInspector> {
  return new Promise((resolve) => {
    function setup() {
      setupInspector();
      resolve(target.__TMAGIC_INSPECTOR__);
    }
    if (!target.__TMAGIC_INSPECTOR__) {
