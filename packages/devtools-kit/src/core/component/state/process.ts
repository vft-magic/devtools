...
function resolveMergedOptions(instance: any) {
 ...
 *
 */
function processProps(instance: any) {
...
function processState(instance: any) {
  const { type } = instance;
  const { props } = type;
  const getters = type.vuex?.getters;
  ...
  return Object.keys(data)
    .filter((key) => !(props && key in props) && !(getters && key in getters) && !(computedDefs && key in computedDefs))
    .map((key) => ({
      key,
      type: 'data',
 ...
 */
function processComputed(instance: any, mergedType: Record<string, unknown>) {
...
function processProvide(instance: any) {
...
function processInject(instance: any, mergedType: Record<string, unknown>) {
  ...
  return keys.map(({ key, originalKey }) => ({
    ...
    value: returnError(() =>
      // eslint-disable-next-line no-nested-ternary
      Object.prototype.hasOwnProperty.call(instance.ctx, key)
        ? instance.ctx[key]
        : Object.prototype.hasOwnProperty.call(instance.provides, originalKey)
...
function processEventListeners(instance: any) {
...
export function processInstanceState(instance: any) {
